...
def load_subject( raw_data_dir, subject_id, T='T3'):
	'''Load subject data.

	Args:
		subject_id: [1-23],T=T1 or T2
...
def write_training_examples(T,T1,T2,  label, original_shape, cut_size, output_file): #T2,
	...
	if T=='T1':
		...
		print('read all data for trainig')
		feature = {
			'T1': _bytes_feature([T1[:, :, :, 0].tostring()]),  # int16
			'T2': _bytes_feature([T2[:,:,:,0].tostring()]), #int16
			'label': _bytes_feature([label[:, :, :, 0].tostring()]),  # uint8
...
def write_validation_examples(T,T1,T2, label, patch_size, cut_size, overlap_stepsize, output_file): #T2,
	...
	if T=='T1':
		...
		patch_ids = prepare_validation(T2, patch_size, overlap_stepsize)
	else:
		print('read all data for validation')
		T1 = T1[cut_size[0]:cut_size[1], cut_size[2]:cut_size[3], cut_size[4]:cut_size[5], :]
		T2 = T2[cut_size[0]:cut_size[1], cut_size[2]:cut_size[3], cut_size[4]:cut_size[5], :]
		patch_ids = prepare_validation(T1, patch_size, overlap_stepsize)
...
def write_prediction_examples(T,T1,T2,  patch_size, cut_size, overlap_stepsize, output_file): #T2,
	...
	if T == 'T1':
		...
		print('read all data for prediction')
		T1 = T1[cut_size[0]:cut_size[1], cut_size[2]:cut_size[3], cut_size[4]:cut_size[5], :]
		T2 = T2[cut_size[0]:cut_size[1], cut_size[2]:cut_size[3], cut_size[4]:cut_size[5], :]
		patch_ids = prepare_validation(T1, patch_size, overlap_stepsize)
	...
	for i in range(len(patch_ids)):
		...
		if T == 'T1':
			...
			print('read all patch data for prediction')
			_T1 = T1[d:d + patch_size, h:h + patch_size, w:w + patch_size, :]
			_T2 = T2[d:d + patch_size, h:h + patch_size, w:w + patch_size, :]
...
def generate_files(T,raw_data_dir, output_path, valid_id, pred_id, patch_size, overlap_stepsize):
	...
	for i in range(1, 24):
		...
		if (i < 11 and not os.path.isfile(train_filename)) or \
			...
			print('Loading data...')
			if T=='T1':
				[_T1, _label] = load_subject(raw_data_dir,i,T)
			elif T=='T2':
				[_T2, _label] = load_subject(raw_data_dir,i,T)
			else:
				[_T1,_T2, _label] = load_subject(raw_data_dir,i,T)
		...
		if not os.path.isfile(train_filename) and i < 11:
			...
			write_training_examples(T,_T1,_T2,  _label, original_shape, cut_size, train_filename) #_T2,
		if i == valid_id:
			if not os.path.isfile(valid_filename):
				print('Create the validation file:')
				write_validation_examples(T,_T1,_T2, _label, patch_size, cut_size, overlap_stepsize, valid_filename) # _T2,
		...
		if i == pred_id:
			if not os.path.isfile(pred_filename):
				...
				write_prediction_examples(T,_T1,_T2, patch_size, cut_size, overlap_stepsize, pred_filename) #_T2,
...
if __name__ == '__main__':
	conf.T='T3'
	for f in range(11,24):
		conf.prediction_id=f
		generate_files(conf.T,
		conf.raw_data_dir,
		conf.data_dir,
		conf.validation_id,
		conf.prediction_id,
